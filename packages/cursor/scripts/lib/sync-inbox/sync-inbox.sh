...
sync_inbox_scan_ts() {
  ...
  if [[ -n "$root" && -f "$root/packages/cursor/scripts/lib/sync-inbox/scan-sync-inbox.ts" ]]; then
    printf '%s/packages/cursor/scripts/lib/sync-inbox/scan-sync-inbox.ts' "$root"
    return 0
  fi
  return 1
}
...
sync_inbox_scan() {
  ...
  source "$(dirname "${BASH_SOURCE[0]}")/../install/hostdime-env.sh"
...
sync_inbox_query_ts() {
  ...
  if [[ -n "$root" && -f "$root/packages/cursor/scripts/lib/sync-inbox/sync-inbox-query.ts" ]]; then
    printf '%s/packages/cursor/scripts/lib/sync-inbox/sync-inbox-query.ts' "$root"
    return 0
  fi
  return 1
}
...
sync_inbox_cards_ts() {
  ...
  if [[ -n "$root" && -f "$root/packages/cursor/scripts/lib/sync-inbox/sync-inbox-cards.ts" ]]; then
    printf '%s/packages/cursor/scripts/lib/sync-inbox/sync-inbox-cards.ts' "$root"
    return 0
  fi
  return 1
}
