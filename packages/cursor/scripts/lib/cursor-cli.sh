...
# Source: packages/cursor/scripts/lib/cursor-cli.sh
...
cursor_cli_resolve_lib() {
  ...
  if [[ -f "$script_dir/lib/cursor-cli.sh" ]]; then
    printf '%s/lib/cursor-cli.sh' "$script_dir"
    return 0
  fi
  ...
  if [[ -n "${HOSTDIME_IA_ROOT:-}" && -f "$HOSTDIME_IA_ROOT/packages/cursor/scripts/lib/cursor-cli.sh" ]]; then
    printf '%s/packages/cursor/scripts/lib/cursor-cli.sh' "$HOSTDIME_IA_ROOT"
    return 0
  fi
